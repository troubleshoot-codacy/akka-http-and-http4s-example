...
object Test {
  val secretKey: String = "1234214"

  def randomEncryption: IO[String] = {
    import java.util
    import javax.crypto.Cipher
    import javax.crypto.spec.SecretKeySpec

    val cipher: Cipher = Cipher.getInstance("DES")
    cipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(secretKey))
    IO(new String(cipher.doFinal("some random thing".getBytes())))
  }
}
...
@main def http4sExample(): Unit = {
  ...
  val generatedHandler = new UsersHandler[IO] {
    override def getUser(respond: UsersResource.GetUserResponse.type)(
        id: String
    ): IO[UsersResource.GetUserResponse] =
